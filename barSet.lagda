...
module barSet {L : Level} (W : PossibleWorlds {L})
       where
...
Bars = Br → Br → Set(lsuc(L))


record 𝔹 (B : Bars) (x : Br) : Set(lsuc(L)) where
  constructor mk𝔹
  field
    bar  : Br
    bars : B x bar
    ext  : {w' : 𝕎·} → bar w' → Σ 𝕎· (λ w → x w × w ⊑· w')
    mon  : {w1 w2 : 𝕎·} → w1 ⊑· w2 → bar w1 → bar w2


{--
{-- Bars and dependent bars --}
∈𝔹 : {B : Bars} {x : Br}  (b : 𝔹 B x) (f : wPred w) → Set(lsuc(L))
...
∩𝔹 : {B : Bars} (isect : Bars∩ B) {x : Br} → 𝔹 B x → 𝔹 B x → 𝔹 B x
∩𝔹 {B} isect {x} (mk𝔹 b1 bars1 ext1 mon1) (mk𝔹 b2 bars2 ext2 mon2) =
  ...
  where
    ...
    ext : {w' : 𝕎·} → bar w' → Σ 𝕎· (λ w → x w × w ⊑· w')
    ext {w'} (w1 , w2 , b₁ , b₂ , e₁ , e₂) = ? --⊑-trans· (𝔹.ext {B} {x} (mk𝔹 b1 bars1 ext1 mon1) {w1} b₁) e₁
...
{-- Top --}
bar∀ : Br → Br
bar∀ x w' = Σ 𝕎· (λ w → x w × w ⊑· w')


Bars∀ : (B : Bars) → Set(lsuc(L))
Bars∀ B = (x : Br) → B x (bar∀ x)


𝔹∀ : {B : Bars} (all : Bars∀ B) (x : Br) → 𝔹 B x
𝔹∀ {B} all x =
  mk𝔹 bar bars ext mon
  where
    bar : Br
...
{-- Union --}
record 𝔹In {B : Bars} {x : Br} (b : 𝔹 B x) : Set(L) where
  constructor mk𝔹In
  field
    w1 : 𝕎·
    e1 : Σ 𝕎· (λ w → x w × w ⊑· w1)
