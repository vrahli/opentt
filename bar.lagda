...
□'-comb : (b : Bar) {w : 𝕎·} {f : wPred w} {g h k : wPredDep f} (i : Bar.□ b w f)
              → ∀𝕎 w (λ w' e' → (z zg zh : f w' e')
                               → g w' e' zg → h w' e' zh → k w' e' z)
              → Bar.□' b w i g → Bar.□' b w i h → Bar.□' b w i k
□'-comb b {w} {f} {g} {h} {k} i aw j₁ j₂ =
  Bar.□'-comb-change b i i i (λ w1 e1 x₁ x₂ x₃ a b → aw w1 e1 x₃ x₁ x₂ a b) j₁ j₂


□'-change : (b : Bar) {w : 𝕎·} {f k : wPred w} {g : wPredDep f} {h : wPredDep k} (i : Bar.□ b w f) (j : Bar.□ b w k)
                → ∀𝕎 w (λ w' e' → (x : f w' e') (y : k w' e') {--→ atBar i w' e' x → atBar j w' e' y--}
                                  → g w' e' x → h w' e' y)
                → Bar.□' b w i g → Bar.□' b w j h
□'-change b {w} {f} {k} {g} {h} i j aw u =
  Bar.□'-comb-change b i i j (λ w1 e1 x₁ x₂ x₃ a b → aw w1 e1 x₁ x₃ a) u u


-- This is a consequence of [∀𝕎-□'-□]
□'-□ : (b : Bar) {w : 𝕎·} {f : wPred w} {h : wPred w}
               → (i : Bar.□ b w f) → Bar.□' b w i (λ w1 e1 z → h w1 e1) → Bar.□ b w h
□'-□ b {w} {f} {h} i q = Bar.∀𝕎-□'-□ b i (λ w1 e1 x {--at--} z → z) q


-- This is a consequence of [□'-comb] for 3 dependent bars
□'3 : (b : Bar) {w : 𝕎·} {f : wPred w} {g h k j : wPredDep f} (i : Bar.□ b w f)
          → ∀𝕎 w (λ w' e' → (z : f w' e') (zg : f w' e') (zh : f w' e') (zk : f w' e')
                             → g w' e' zg → h w' e' zh → k w' e' zk → j w' e' z)
          → Bar.□' b w i g → Bar.□' b w i h → Bar.□' b w i k → Bar.□' b w i j
□'3 b {w} {f} {g} {h} {k} {j} i imp ig ih ik = c
  where
    ip : Bar.□' b w i (λ w1 e1 z → Σ (f w1 e1) λ zg → Σ (f w1 e1) λ zh → g w1 e1 zg × h w1 e1 zh)
