...
module indKripke {L : Level} (W : PossibleWorlds {L}) --(M : Mod(W))
...
Same as ind.lagda but for a Kripke modality, and without the TERMINATING pragma.
...
data <TypeStep where
  ...
  <TypeTCONST : (u : univs) (w : 𝕎·) (T1 T2 : CTerm) (A1 A2 : CTerm)
                (c₁ : T1 #⇛ (#TCONST A1) at w)
                (c₂ : T2 #⇛ (#TCONST A2) at w)
                (eqtA : ∀𝕎 w (λ w' _ → eqTypes u w' A1 A2))
                (exta : (a b : CTerm) → wPredExtIrr (λ w e → eqInType u w (eqtA w e) a b))
...
{--  <TypeDUM₀ : (u : univs) (w : 𝕎·) (T1 T2 : CTerm) (A1 A2 : CTerm)
             ...
             (exta : (a b : CTerm) → wPredExtIrr (λ w e → eqInType u w (eqTypes-mon u eqtA w e) a b))
             → <TypeStep {u} (eqtA) {u} {w} {T1} {T2} (EQTDUM A1 A2 c₁ c₂ (eqTypes-mon u eqtA) exta)--}
{--  <TypeDUM : (u : univs) (w : 𝕎·) (T1 T2 : CTerm) (A1 A2 : CTerm)
             (c₁ : T1 #⇛ #DUM A1 at w)
             (c₂ : T2 #⇛ #DUM A2 at w)
             (eqtA : ∀𝕎 w (λ w' _ → eqTypes u w' A1 A2))
             (exta : (a b : CTerm) → wPredExtIrr (λ w e → eqInType u w (eqtA w e) a b))
             (w' : 𝕎·) (e' : w ⊑· w')
             → <TypeStep {u} (eqtA w' e') {u} {w} {T1} {T2} (EQTDUM A1 A2 c₁ c₂ eqtA exta)--}
...
TCONSTeq-ext : {u : univs} {w : 𝕎·} {A1 A2 : CTerm}
               {eqta : ∀𝕎 w (λ w' _ → eqTypes u w' A1 A2)}
               {w' : 𝕎·} {e1 e2 : w ⊑· w'} {a b : CTerm}
               (exta : (a b : CTerm) → wPredExtIrr (λ w e → eqInType u w (eqta w e) a b))
               → TCONSTeq (eqInType u w' (eqta w' e1)) w' a b
               → TCONSTeq (eqInType u w' (eqta w' e2)) w' a b
