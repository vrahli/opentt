...
abstract
  ℕ→Term→ℕ₂ : (t₁ t₂ : Term) (op : Term → Term → Term) (j : ℕ)
                  ...
                  → ℕ→Term (Term→ℕ t₁) ≡ t₁
                  → ℕ→Term (Term→ℕ t₂) ≡ t₂
                  → op (comp-ind-ℕ-aux2 (λ _ → Term) ℕ→Term-aux ((suc j) + pairing (Term→ℕ t₁ , Term→ℕ t₂) * #cons ∸ 1) (pairing→₁ ((j + pairing (Term→ℕ t₁ , Term→ℕ t₂) * #cons ∸ j) / #cons)) (≤-trans (s≤s-inj (≤-<-trans (pairing→₁≤ ((j + pairing (Term→ℕ t₁ , Term→ℕ t₂) * #cons ∸ j) / #cons)) (suc-/≤ ((suc j) + pairing (Term→ℕ t₁ , Term→ℕ t₂) * #cons) (suc j) (λ ())))) (s≤s-inj ≤-refl)))
                        (comp-ind-ℕ-aux2 (λ _ → Term) ℕ→Term-aux ((suc j) + pairing (Term→ℕ t₁ , Term→ℕ t₂) * #cons ∸ 1) (pairing→₂ ((j + pairing (Term→ℕ t₁ , Term→ℕ t₂) * #cons ∸ j) / #cons)) (≤-trans (s≤s-inj (≤-<-trans (pairing→₂≤ ((j + pairing (Term→ℕ t₁ , Term→ℕ t₂) * #cons ∸ j) / #cons)) (suc-/≤ ((suc j) + pairing (Term→ℕ t₁ , Term→ℕ t₂) * #cons) (suc j) (λ ())))) (s≤s-inj ≤-refl)))
                     ≡ op t₁ t₂
  ℕ→Term→ℕ₂ t₁ t₂ op j ≡op ind₁ ind₂ =
    ≡op (comp-ind-ℕ-aux2≡ℕ→Term₁- {j + pairing (Term→ℕ t₁ , Term→ℕ t₂) * #cons} {j} {Term→ℕ t₁} {Term→ℕ t₂} (≤-trans (s≤s-inj (≤-<-trans (pairing→₁≤ ((j + pairing (Term→ℕ t₁ , Term→ℕ t₂) * #cons ∸ j) / #cons)) (suc-/≤ ((suc j) + pairing (Term→ℕ t₁ , Term→ℕ t₂) * #cons) (suc j) (λ ())))) (s≤s-inj ≤-refl)) t₁ ind₁)
        (comp-ind-ℕ-aux2≡ℕ→Term₂- {j + pairing (Term→ℕ t₁ , Term→ℕ t₂) * #cons} {j} {Term→ℕ t₁} {Term→ℕ t₂} (≤-trans (s≤s-inj (≤-<-trans (pairing→₂≤ ((j + pairing (Term→ℕ t₁ , Term→ℕ t₂) * #cons ∸ j) / #cons)) (suc-/≤ ((suc j) + pairing (Term→ℕ t₁ , Term→ℕ t₂) * #cons) (suc j) (λ ())))) (s≤s-inj ≤-refl)) t₂ ind₂)
...
abstract
  ℕ→Term→ℕ₃ : (t₁ t₂ t₃ : Term) (op : Term → Term → Term → Term) (j : ℕ)
                     (≡op : {a b c d e f : Term} → a ≡ b → c ≡ d → e ≡ f → op a c e ≡ op b d f)
                      ...
                      → ℕ→Term (Term→ℕ t₂) ≡ t₂
                      → ℕ→Term (Term→ℕ t₃) ≡ t₃
                      → op (comp-ind-ℕ-aux2 (λ _ → Term) ℕ→Term-aux (suc j + pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons ∸ 1) (pairing3→₁ ((j + (pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons) ∸ j) / #cons)) (≤-trans (s≤s-inj (≤-<-trans (pairing3→₁≤ ((j + (pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons) ∸ j) / #cons)) (suc-/≤ (suc j + pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons) (suc j) (λ ())))) (s≤s-inj ≤-refl)))
                            (comp-ind-ℕ-aux2 (λ _ → Term) ℕ→Term-aux (suc j + pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons ∸ 1) (pairing3→₂ ((j + (pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons) ∸ j) / #cons)) (≤-trans (s≤s-inj (≤-<-trans (pairing3→₂≤ ((j + (pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons) ∸ j) / #cons)) (suc-/≤ (suc j + pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons) (suc j) (λ ())))) (s≤s-inj ≤-refl)))
                            (comp-ind-ℕ-aux2 (λ _ → Term) ℕ→Term-aux (suc j + pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons ∸ 1) (pairing3→₃ ((j + (pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons) ∸ j) / #cons)) (≤-trans (s≤s-inj (≤-<-trans (pairing3→₃≤ ((j + (pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons) ∸ j) / #cons)) (suc-/≤ (suc j + pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons) (suc j) (λ ())))) (s≤s-inj ≤-refl)))
                         ≡ op t₁ t₂ t₃
  ℕ→Term→ℕ₃ t₁ t₂ t₃ op j ≡op ind₁ ind₂ ind₃ =
    ≡op (comp-ind-ℕ-aux2≡ℕ→Term3₁- {j + pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons} {j} {Term→ℕ t₁} {Term→ℕ t₂} {Term→ℕ t₃} (≤-trans (s≤s-inj (≤-<-trans (pairing3→₁≤ ((j + (pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons) ∸ j) / #cons)) (suc-/≤ (suc j + pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons) (suc j) (λ ())))) (s≤s-inj ≤-refl)) t₁ ind₁)
        (comp-ind-ℕ-aux2≡ℕ→Term3₂- {j + pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons} {j} {Term→ℕ t₁} {Term→ℕ t₂} {Term→ℕ t₃} (≤-trans (s≤s-inj (≤-<-trans (pairing3→₂≤ ((j + (pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons) ∸ j) / #cons)) (suc-/≤ (suc j + pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons) (suc j) (λ ())))) (s≤s-inj ≤-refl)) t₂ ind₂)
        (comp-ind-ℕ-aux2≡ℕ→Term3₃- {j + pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons} {j} {Term→ℕ t₁} {Term→ℕ t₂} {Term→ℕ t₃} (≤-trans (s≤s-inj (≤-<-trans (pairing3→₃≤ ((j + (pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons) ∸ j) / #cons)) (suc-/≤ (suc j + pairing3 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃) * #cons) (suc j) (λ ())))) (s≤s-inj ≤-refl)) t₃ ind₃)
...
abstract
  ...
  ℕ→Term→ℕ₄ t₁ t₂ t₃ t₄ op j ≡op ind₁ ind₂ ind₃ ind₄ =
    ≡op (comp-ind-ℕ-aux2≡ℕ→Term4₁- {j + pairing4 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃ , Term→ℕ t₄) * #cons} {j} {Term→ℕ t₁} {Term→ℕ t₂} {Term→ℕ t₃} {Term→ℕ t₄} (≤-trans (s≤s-inj (≤-<-trans (pairing4→₁≤ ((j + (pairing4 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃ , Term→ℕ t₄) * #cons) ∸ j) / #cons)) (suc-/≤ (suc j + pairing4 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃ , Term→ℕ t₄) * #cons) (suc j) (λ ())))) (s≤s-inj ≤-refl)) t₁ ind₁)
        (comp-ind-ℕ-aux2≡ℕ→Term4₂- {j + pairing4 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃ , Term→ℕ t₄) * #cons} {j} {Term→ℕ t₁} {Term→ℕ t₂} {Term→ℕ t₃} {Term→ℕ t₄} (≤-trans (s≤s-inj (≤-<-trans (pairing4→₂≤ ((j + (pairing4 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃ , Term→ℕ t₄) * #cons) ∸ j) / #cons)) (suc-/≤ (suc j + pairing4 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃ , Term→ℕ t₄) * #cons) (suc j) (λ ())))) (s≤s-inj ≤-refl)) t₂ ind₂)
        (comp-ind-ℕ-aux2≡ℕ→Term4₃- {j + pairing4 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃ , Term→ℕ t₄) * #cons} {j} {Term→ℕ t₁} {Term→ℕ t₂} {Term→ℕ t₃} {Term→ℕ t₄} (≤-trans (s≤s-inj (≤-<-trans (pairing4→₃≤ ((j + (pairing4 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃ , Term→ℕ t₄) * #cons) ∸ j) / #cons)) (suc-/≤ (suc j + pairing4 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃ , Term→ℕ t₄) * #cons) (suc j) (λ ())))) (s≤s-inj ≤-refl)) t₃ ind₃)
        (comp-ind-ℕ-aux2≡ℕ→Term4₄- {j + pairing4 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃ , Term→ℕ t₄) * #cons} {j} {Term→ℕ t₁} {Term→ℕ t₂} {Term→ℕ t₃} {Term→ℕ t₄} (≤-trans (s≤s-inj (≤-<-trans (pairing4→₄≤ ((j + (pairing4 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃ , Term→ℕ t₄) * #cons) ∸ j) / #cons)) (suc-/≤ (suc j + pairing4 (Term→ℕ t₁ , Term→ℕ t₂ , Term→ℕ t₃ , Term→ℕ t₄) * #cons) (suc j) (λ ())))) (s≤s-inj ≤-refl)) t₄ ind₄)
